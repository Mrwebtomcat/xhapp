...
<template>
  <div class="patchdetailcon">
  	   <navigBar class="navbar" :ShowUsericon="false"  :titleText="'我的会员'" @checkMe="clickMe"></navigBar>
 		<div class="ouside">
			<div class="vip_banner">
				<div v-if="vUserData.n_isvip" class="time_tip">
					会员{{vUserData.dt_endvip}}到期
				</div>
		...
		<Dilogoinfo v-if="isShow" @zfbpay="zfbpay" @weixinpay="weixinpay" @closeDilog="guanbi" />
		<div class="noticbox">
			<p>服务说明：</p>
...
import Dilogoinfo from '@/components/Dilogoinfo';//状态栏组件
	...
	//		alert("这里不能点击返回，去掉返回按钮")
		...
		getFuWu:function(dataarr){
			...
			.then(rd => {
				...
				let datas =  rd.data.filter((item,index,arr)=>item.n_sex==this.vUserData.n_sex);
				this.itemList=dataarr.concat(...datas);
